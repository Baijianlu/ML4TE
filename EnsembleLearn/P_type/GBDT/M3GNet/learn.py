...
if __name__ == '__main__':
    ...
    df1.loc[:,["m_cond_v","m_dos_v"]] = df1.loc[:,["m_cond_v","m_dos_v"]].apply(np.abs)
    df2.loc[:,["m_cond_v","m_dos_v"]] = df2.loc[:,["m_cond_v","m_dos_v"]].apply(np.abs)
    df1.drop(columns=['class'],inplace=True)
    df2.drop(columns=['class'],inplace=True) 
    X_train = df1.to_numpy()
    X_test = df2.to_numpy()
 
    estimators = [('scaler', StandardScaler()), ('clf', GradientBoostingClassifier(subsample=0.5, min_weight_fraction_leaf=0.0043, random_state=0))] #
    pipe = Pipeline(estimators)                     

    param_grid = {"clf__n_estimators": [100,200,250,300,350,400,500], "clf__learning_rate": [0.06,0.07,0.08,0.09,0.1,0.11,0.12], "clf__max_depth": [3,4,5,6]}
