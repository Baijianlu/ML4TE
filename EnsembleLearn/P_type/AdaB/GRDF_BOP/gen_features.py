...
from matminer.featurizers.site import GeneralizedRadialDistributionFunction, BondOrientationalParameter
...
if __name__ == '__main__':
    ...
    GRDF_feat = GeneralizedRadialDistributionFunction.from_preset('trigonometric',spacing=0.3,cutoff=6)
    BOP_feat = BondOrientationalParameter(max_l=20, compute_w=True) 
    featurizer = MultipleFeaturizer([
        StructureComposition(Stoichiometry()),
        StructureComposition(ElementProperty.from_preset("magpie")),
        StructureComposition(ValenceOrbital(props=['frac'])),
        StructureComposition(IonProperty(fast=True)),
        SiteStatsFingerprint(GRDF_feat),
        SiteStatsFingerprint(BOP_feat),
        DensityFeatures()
    ])
    PD_train = featurizer.featurize_dataframe(df1, "structure", ignore_errors=True)
    PD_test = featurizer.featurize_dataframe(df2, "structure", ignore_errors=True)

    excludes = ['index','name','structure_opt','p_con','p_PF','structure','compound possible']
