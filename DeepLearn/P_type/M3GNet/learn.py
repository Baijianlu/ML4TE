...
class MyM3GNetClassifier(d2l.Module):
    ...
    def forward(self, g, lat, l_g, state_attr=None):
        ...
        return self.net(g=g, l_g=l_g, state_attr=state_attr)
...
#        return torch.optim.SGD(self.parameters(), self.lr)
        ...
        return torch.optim.Adam(params_wonode, lr=self.lr, eps=1e-8, weight_decay=0.0048)

    def configure_optimizers_finetuning(self):
        params_1x = [param for name, param in self.net.named_parameters() if re.match('final_layer', name) is None]
        return torch.optim.Adam([{'params': params_1x}, 
                                 {'params': self.net.final_layer.parameters(), 'lr': self.lr*10}], 
                                lr=self.lr, eps=1e-8)
    ...
    def training_step(self, batch):
        ...
        l = self.loss(self(g=g, lat=lat, l_g=l_g, state_attr=state_attr), labels)
        return l, labels.shape[0]

    def validation_step(self, batch):
        g, lat, l_g, state_attr, labels = batch
        y_hat = self(g=g, lat=lat, l_g=l_g, state_attr=state_attr)
...
class MyTrainer(d2l.Trainer):
    ...
    def prepare_model_finetuning(self, model, dims, dropout=0.0, activation_type="swish"):
        try:
        ...
        model.net.final_layer = MLP(
                                    dims=dims,
                                    activation=activation,
                                    activate_last=False,
                                    dropout=dropout,
                                    )
        xavier_init(model.net.final_layer)
        model.net.task_type = "classification"
        model.net.sigmoid = nn.Sigmoid()
...
if __name__ == '__main__':
    '''Comments '''
    df1 = pd.read_excel("X_train_p.xlsx", sheet_name='Sheet1',index_col=False)
    df2 = pd.read_excel("X_test_p.xlsx", sheet_name='Sheet1',index_col=False)
    ...
    train_state = df1.loc[:, ['N_v','m_cond_v','m_dos_v','bandgap','defconst_v']].to_numpy()
    val_state = df2.loc[:, ['N_v','m_cond_v','m_dos_v','bandgap','defconst_v']].to_numpy() 

    converter = Structure2Graph(element_types=DEFAULT_ELEMENTS, cutoff=5.0)
    train_dataset = M3GNetDataset(threebody_cutoff=5.0, structures=train_structures, converter=converter, labels={"class": train_labels}, graph_labels=train_state, name='train_set')
    ClearFiles()
    val_dataset = M3GNetDataset(threebody_cutoff=5.0, structures=val_structures, converter=converter, labels={"class": val_labels}, graph_labels=val_state, name='val_set')
    ClearFiles()
   
    data = MP_dataset((train_dataset, val_dataset, None), batch_size=64, num_workers=1)
    model = MyM3GNetClassifier(lr=0.001, dropout=0.0)
    model_loaded = matgl.load_model("M3GNet-MP-2018.6.1-Eform")
    model.net.embedding.layer_node_embedding = model_loaded.model.embedding.layer_node_embedding
    dims = [21,8,8,1]
    trainer = MyTrainer(max_epochs=550, num_gpus=0)
    trainer.fit(model, data, dims, filepath='/home/taofan/work/MachineLearn/pytorch/M3GNet/P_type_unit8/WD01', dropout=0.0)
...
#    df2.to_excel('X_test_p_withpreds.xlsx', index_label='index', merge_cells=False)
#    model.net.save(path=".") 
