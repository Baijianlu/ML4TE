...
#        return torch.optim.SGD(self.parameters(), self.lr, momentum=0.9, weight_decay=0.0001)
        params_wonode = [param for name, param in self.net.named_parameters() if re.match('embedding.layer_node_embedding', name) is None]
        return torch.optim.Adam(params_wonode, lr=self.lr, eps=1e-8, weight_decay=0.0056)
...
if __name__ == '__main__':
    ...
    train_dataset = MEGNetDataset(structures=train_structures, converter=converter, labels={"class": train_labels}, graph_labels=train_state, initial=0.0, final=6.0, num_centers=100, width=0.5, name='train_set')
    ClearFiles()
    val_dataset = MEGNetDataset(structures=val_structures, converter=converter, labels={"class": val_labels}, graph_labels=val_state, initial=0.0, final=6.0, num_centers=100, width=0.5, name='val_set')
    ClearFiles()
   
    data = MP_dataset((train_dataset, val_dataset, None), batch_size=64, num_workers=1)
    model = MyMEGNetClassifier(lr=0.001, dropout=0.0)
    model_loaded = matgl.load_model("MEGNet-MP-2018.6.1-Eform")
    ...
    trainer.fit(model, data, dims, filepath='/home/taofan/work/MachineLearn/pytorch/MEGNet/P_type_unit8/WD04', dropout=0.0)
